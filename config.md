...
| `org.opencontainers.runtime.exec.isolated-cpu-affinity-transition` | Controls the CPU affinity of a process created with an exec into an existing container. This annotation supports these values:<br>`temporary`: to temporarily set the process CPU affinity to the first isolated CPU of the container cgroup cpuset<br>`definitive`: to definitively (i.e. permanently) set the process CPU affinity to the first isolated CPU of the container cgroup cpuset |
